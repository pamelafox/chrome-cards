  ...
  for region_code, region_data in list(data_by_code.items()):
    ...
    for polygon in coordinates:
      if len(polygon[0]) == 2:
        ...
        draw_area(polygon[0])
    capital = staticmaps.create_latlng(float(region_data["lat"]), float(region_data["long"]))
...
def generate_state_maps():
  ...
  generate_maps(state_data, "uscapitals/images/us_capitals_")
...
for country_feature in country_features:
   ...
   country_match = [code for code, country_data in country_data_by_code.items() if country_data["name"] == name]
   ...
   try:
     ...
     print(f"Missing country {country_code}, {country_feature['properties']['ADMIN']}")
